...
const addAvatar = require("../use-cases/add-avatar.js");
...
router.patch("/users/:id/avatar", authGuard, (req, res) => {
    // Obtener el usuario con id req.params.id
    res.send("Detalle usuario");
});
router.post(
    ...
    fileUpload(),
    handleAsyncError(async (req, res) => {
        //Agregar un nuevo avatar al userid
        await addAvatar(req.params.id, req.currentUser.id, req.files.photo);
